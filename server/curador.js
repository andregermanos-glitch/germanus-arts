...
async function searchMet(query) {
  ...
  try {
    ...
    // Adiciona "painting" ao query para afinar resultados
    ...
    for (const id of ids) {
      ...
      try {
        ...
        // Filtro de tipo: rejeitar esculturas, gravuras, cerâmica, mobiliário, etc.
...
async function searchCleveland(query) {
  ...
  try {
    ...
    // Cleveland API suporta &type=painting — filtro directo

    const obj = (d.data || []).find(o => {

      if (!o.images?.web?.url) return false;

      // Verificar que o tipo é pintura
...
async function searchAIC(query) {
  ...
  try {
    ...
    // Tipos aceites: apenas pinturas — rejeitar esculturas, gravuras, têxteis, etc.
    ...
    const obj = (d.data || []).find(o => {
      ...
      // Aceitar se tipo é pintura ou se não há informação de tipo (assume pintura)
